...
template <typename T, int key_or_value>
__global__ void appendKeyValueCache(T *kv_dst, // [num layers, bs, kv head num, max_seq_len, head_size, 2 (key/value)]
...
void launchConcatKVCache(TensorWrapper<T> *k_src, // from qkv bias and rope {batch_size, kv_head_num, max_q_len, head_size}
    ...
    size_t layer_offset = layer * batch_size * kv_head_num * max_seq_len * head_size * 2;

    dim3 grid(max_q_len, batch_size, kv_head_num); // last dimension for key/value
    dim3 block(head_size);

    appendKeyValueCache<T, 0><<<grid, block>>>(k_dst->data,
                                               layer_offset,
                                               k_src->data,
                                               kv_head_num,
                                               head_size,
    ...
    appendKeyValueCache<T, 1><<<grid, block>>>(v_dst->data,
                                               layer_offset,
                                               v_src->data,
                                               kv_head_num,
                                               head_size,
...
template void launchConcatKVCache(TensorWrapper<float> *k_src,
...
template void launchConcatKVCache(TensorWrapper<half> *k_src,
