...
//    [num_layers, bs, kv head num, max_seq_len, head_size]
...
template <typename T>
__global__ void repeatKVCache(T *kv_dst,
                              const T *kv_src,
    ...
    const auto val_src = kv_src + layer_offset;
    const auto val_dst = kv_dst;
    ...
    if (v_seq_len_id < seq_len) {
        const int src_idx = batch_id * (head_num / q_head_per_kv) * head_size * max_seq_len + // B
                            head_id / q_head_per_kv * head_size * max_seq_len +               // H
                            v_seq_len_id * head_size +                                        // s
                            v_head_size_id;                                                  // D/x

        const int dst_idx = batch_id * head_num * head_size * max_k_len + // B
...
void launchRepeatKVCache(TensorWrapper<T> *k_cache_src, //{num_layers, batch_size, kv_head_num, max_seq_len, head_size}
    ...
    dim3 grid((max_k_len * head_size + block_size - 1) / block_size, batch_size, head_num);
    repeatKVCache<T><<<grid, block>>>(v_cache_dst->data,
                                      v_cache_src->data,
                                      layer_offset,
                                      head_num,
                                      q_head_per_kv,
    ...
    repeatKVCache<T><<<grid, block>>>(k_cache_dst->data,
                                     k_cache_src->data,
                                     layer_offset,
                                     head_num,
                                     q_head_per_kv,
