...
__device__ T blockReduceSum(T val) {
    ...
    T sum = tid < warp_num ? warp_sum[tid] : reinterpret_cast<T>(0);
...
template <typename T> // only for float
__global__ void RMSNorm(T *decoder_out, // [num tokens, q_hidden_units]
                        T *decoder_residual, T *weights, 
                        float eps, int num_tokens, int hidden_units) {
    const int vec_size = Vec<T>::size;
    using Vec_t = typename Vec<T>::Type;
    float thread_sum = 0.0f;
    auto dout = reinterpret_cast<Vec_t *>(decoder_out + blockIdx.x * hidden_units);
    auto rsd = reinterpret_cast<Vec_t *>(decoder_residual + blockIdx.x * hidden_units);
    ...
    for (int idx = threadIdx.x; idx < hidden_units / vec_size; idx += blockDim.x) {
        ...
        dout[idx].x = out.x * inv_mean * vec_weights[idx].x;
        dout[idx].y = out.y * inv_mean * vec_weights[idx].y;
        dout[idx].z = out.z * inv_mean * vec_weights[idx].z;
        dout[idx].w = out.w * inv_mean * vec_weights[idx].w;
    }
}

template<>
__global__ void RMSNorm(half *decoder_out, // [num tokens, q_hidden_units]
                        half *decoder_residual, half *weights, 
                        float eps, int num_tokens, int hidden_units) {
    int vec_size = Vec<half>::size;
    using Vec_t = typename Vec<half>::Type;
    auto dout = reinterpret_cast<Vec_t*>(decoder_out + blockDim.x * hidden_units);
    Vec_t *rsd;
    ...
    auto vec_weights = reinterpret_cast<Vec_t*>(weights);
    for(int i = threadIdx.x; i < hidden_units / vec_size; i += blockDim.x) {
        Vec_t dout_half2 =dout[i];
        dout[i].x = vec_weights[i].x * __float2half(__half2float(dout_half2.x) * inv_fenmu);
        dout[i].y = vec_weights[i].y * __float2half(__half2float(dout_half2.y) * inv_fenmu);
    }
}
