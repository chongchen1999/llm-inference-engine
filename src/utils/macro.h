...
// Macro to check CUDA calls and provide detailed error messages
#define CHECK(call)                                   \
do {                                                  \
...
[[noreturn]] inline void throwRuntimeError(const char* const file, int const line, const std::string& info = "") {
    throw std::runtime_error(
...
inline void llmAssert(bool result, const char* const file, int const line, const std::string& info = "") {
    if (!result) {
