...
void CPUFusedResidualAndRMSNorm(float *h_residual, float *h_decoder_out, float *h_bias, 
...
int main() {
    ...
    TensorWrapper<float> *residual_tensor = new TensorWrapper<float>(Device::GPU, type_float, {num_tokens, hidden_units}, d_residual);                                                                        
    BaseWeight<float> norm;
    LayerNormWeight<float> scale;
    scale.gamma = d_scale;

    std::cout << "Before launch kernel" << std::endl;
    launchFusedAddBiasResidualRMSNorm(residual_tensor, decoder_out_tensor, norm, d_scale, eps);
    ...
    CPUFusedResidualAndRMSNorm(h_residual, CPUout, h_bias, h_scale, eps, hidden_units, num_tokens);
