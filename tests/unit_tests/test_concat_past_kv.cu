...
#include <cuda.h>
#include "../../src/kernels/includes/concat_past_kv.cuh"

// ()note:
// there is no concat kv cpu kernel implementation now
// we compare the kernel correctness by eyes and result print infos
...
int main() {
    ...
    const int kv_size = batch_size * max_q_len * kv_head_num * head_size;
    const int layer_offset = batch_size * max_seq_len * kv_head_num * head_size;
    const int kvcache_size = layer_offset;

    float *h_k_src = (float *)malloc(sizeof(float) * kv_size);
    float *d_k_src;
    cudaMalloc((void **)&d_k_src, sizeof(float) * kv_size);

    float *h_v_src = (float *)malloc(sizeof(float) * kv_size);
    float *d_v_src;
    ...
    auto in_ksrc = new TensorWrapper<float>(Device::GPU, type, {batch_size, kv_head_num, max_q_len, head_size}, d_k_src);
    auto in_vsrc = new TensorWrapper<float>(Device::GPU, type, {batch_size, kv_head_num, max_q_len, head_size}, d_v_src);
    auto layer_id = new TensorWrapper<int>(Device::CPU, type_int, {batch_size}, h_layer_id);
    auto cur_q_len = new TensorWrapper<int>(Device::GPU, type_int, {batch_size}, dcur_query_length);
    auto history_len = new TensorWrapper<int>(Device::GPU, type_int, {batch_size}, dhistory_length);
    auto out_kdst = new TensorWrapper<float>(Device::GPU, type, {batch_size, kv_head_num, max_seq_len, head_size}, d_k_dst);
    auto out_vdst = new TensorWrapper<float>(Device::GPU, type, {batch_size, kv_head_num, max_seq_len, head_size}, d_v_dst);
    ...
    for (int i = batch_size * kv_head_num * head_size; i < batch_size * max_seq_len * kv_head_num * head_size; ++i) {
        printf("index = %d\n", i);
        printf("res k = %f\n", h_k_dst[i]);
