...
void CPUSwiGLU(T *input, T *output, int batch_size, int intermedia_size) {
    ...
    for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
        for (int i = 0; i < intermedia_size; ++i) {
            ...
            silu_out = static_cast<float>(input[offset1]) / (1.0f + expf(-static_cast<float>(input[offset1])));
            output[out_offset] = static_cast<T>(silu_out * static_cast<float>(input[offset2]));
        }
    }
}
