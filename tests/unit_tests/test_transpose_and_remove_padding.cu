#include "../../src/kernels/includes/transpose_and_remove_padding.cuh"
#include <iostream>

// [b,h,s,d] => [b,s,h,d] => [num tokens,h,d]
// padding_offset.shape = [num_tokens]
// Compare the kernel correctness by visual inspection and printed results
// Use `./test_fused_trans_remv_pad` to test the fp32 kernel
...
int main() {
    ...
    std::cout << "batch_size=" << batch_size << "  num_tokens=" << num_tokens << std::endl;
    ...
    // Note: remember to memcpy from device to host and define the correct copy size (multiplying by sizeof(dtype)), or it will cause segmentation fault
    cudaMemcpy(h_out, out->data, sizeof(float) * out_size, cudaMemcpyDeviceToHost);

    for (int i = 0; i < out_size; ++i) {
        printf("after transpose and remove padding, out[%d] = %f\n", i, h_out[i]);
    }
